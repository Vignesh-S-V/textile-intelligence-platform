...
MIN_DATE='2021-01-01'
HEADERS={'User-Agent':'Textile-Intelligence-Platform market-report bot/1.2'}
SOURCES=[{'name':'Textile Today','short':'TEXTILE_TODAY','feed':'https://www.textiletoday.com.bd/feed/?s=cotton%20yarn','search':'https://www.textiletoday.com.bd/?s=cotton+yarn'}]
...
def parse_article(url,source_short,source_name,published_hint=None):
    ...
    for m in PRICE_RANGE_RE.finditer(text):
        spans.append((m.start(),m.end()));window=text[max(0,m.start()-220):min(len(text),m.end()+220)];cm=COUNT_RE.search(window)
...
def textile_today():
    ...
    for source in SOURCES:
        # RSS is preferred; the site search is a fallback because some WordPress
        # deployments do not expose the filtered RSS feed consistently.
        try:
            soup=BeautifulSoup(get(source['feed']).decode('utf-8','ignore'),'xml')
        ...
        try:
            soup=BeautifulSoup(get(source['search']).decode('utf-8','ignore'),'html.parser')
            links=[]
            for a in soup.find_all('a',href=True):
                link=urljoin(source['search'],a['href']);label=clean(a.get_text(' ',strip=True))
                if link.startswith('https://www.textiletoday.com.bd/') and link not in links and link not in seen_links and 'yarn' in (link+' '+label).lower():links.append(link)
            print('Textile Today search links:',len(links))
            for link in links[:50]:
                try:rows.extend(parse_article(link,source['short'],source['name']))
                except Exception as e:print('Search article failed:',link,e)
