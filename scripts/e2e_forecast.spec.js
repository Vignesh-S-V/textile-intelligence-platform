...
async function selectKnownGoodFilters(page) {
...
test('forecast renders through the UI without console errors', async ({ page }) => {
  ...
  await selectKnownGoodFilters(page);
  ...
  // Regression test for the original frontend bug: a selection with exactly
  // one calendar month must still call the API and render a short-history forecast.
  await page.locator('#year').selectOption('2021');
  await expect.poll(async () => page.locator('#month option[value="1"]').count(), { timeout: 60_000 }).toBe(1);
